...
ws.onopen = () => {
    ...
    ws.send(JSON.stringify({ type: 'identify', message: 'admin' }));
};
...
// 관리자 페이지에서 답변을 전송하는 함수
function sendResponse() {
    const responseInput = document.getElementById('responseInput');
    const response = responseInput.value;
    if (ws.readyState === WebSocket.OPEN && response) {
        ws.send(JSON.stringify({ type: 'answer', text: response }));
        console.log('관리자 페이지에서 서버로 답변을 보냈습니다: ', response);
        responseInput.value = '';
    } else {
        console.error('웹소켓이 열려있지 않거나 답변이 비어있습니다.');
    }
}